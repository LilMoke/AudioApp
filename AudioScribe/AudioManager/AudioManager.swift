...
private let logger = Logger(subsystem: "AudioScribe", category: "AudioManager")
...
extension AudioManager {
	...
	private func transcribeSegment(segmentID: UUID) async {
		let keepAudioClips = UserDefaults.standard.bool(forKey: "keepAudioClips")
		let transcriptionModeRaw = UserDefaults.standard.string(forKey: "transcriptionMode") ?? TranscriptionMode.apple.rawValue
		let transcriptionMode = TranscriptionMode(rawValue: transcriptionModeRaw) ?? .apple
			...
			switch transcriptionMode {
				case .apple:
					do {
						...
						logger.info("Using Apple Speech for transcription.")
				...
				case .openai:
					do {
						...
						logger.warning("OpenAI Whisper failed, falling back to Apple Speech: \(error.localizedDescription)")
						do {
							let appleService = AppleSpeechRecognizerService(onError: onError)
							text = try await appleService.transcribeAudioFile(url: segment.fileURL)
